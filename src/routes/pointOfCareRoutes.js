...
router.post('/ct-scan',authMiddleware, upload.single('ctScanDocument'),renameUploadedFile('name'), pointOfCareController.createCtScanRecord);
router.get('/ct-scan/:patientId', authMiddleware, pointOfCareController.getCtScanRecords);
router.post('/blood-gas', authMiddleware, upload.single('bloodGasDocument'),renameUploadedFile('name'), pointOfCareController.createBloodGasRecord);
